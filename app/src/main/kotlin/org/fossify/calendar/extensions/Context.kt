...
import org.fossify.calendar.helpers.HierarchyHelper
import org.fossify.calendar.helpers.HierarchyInfo
    ...
    eventsHelper.getEvents(now, now + YEAR, event.id!!, false) { events ->
        if (events.isNotEmpty()) {
            for (curEvent in events) {
                if (curEvent.shouldStrikeThrough()) {
                    // skip scheduling reminders for completed items
                    continue
                }
...
fun Context.getEventListItems(
    events: List<Event>,
    contactEvents: List<Event> = events,
    ...
    // 0. Ensure input events are sorted - Time then Hierarchy Level
    val sortedEvents = events.sortedWith(compareBy<Event> {
        if (it.getIsAllDay()) Formatter.getDayStartTS(Formatter.getDayCodeFromTS(it.startTS)) - 1 else it.startTS
    }.thenBy { HierarchyHelper.extractHierarchy(it.description)?.level ?: 1 }
     .thenBy { it.endTS }
     .thenBy { it.title })
    ...
    // 2. Sort context by time - Primary sorting is Time, then Hierarchy Level (Level 1 first)
    ...
    }.thenBy { it.second?.level ?: 1 }
     .thenBy { (event, _) ->
        if (event.getIsAllDay()) Formatter.getDayEndTS(Formatter.getDayCodeFromTS(event.endTS)) else event.endTS
    }.thenBy { it.first.title })

    // 3. Build the Parent-Child Map using Strict + Suffix logic
    val childrenMap = mutableMapOf<Long, MutableList<Pair<Event, HierarchyInfo>>>()
    val prospectiveParentsByLevel = mutableMapOf<Int, MutableList<Pair<Event, HierarchyInfo>>>()
    ...
    contextSorted.forEach { (event, hierarchy) ->
        if (hierarchy != null) {
            ...
            if (level > 1) {
                ...
                val prospectiveParents = prospectiveParentsByLevel[parentLevel] ?: mutableListOf()
                
                // Find nearest parent (last in the list, as it's sorted by time)
                val parentMatch = prospectiveParents.lastOrNull { (pEvent, pHierarchy) ->
                    val timeLimit = DateTime(pEvent.startTS * 1000L).plusYears(1).seconds()
                    event.startTS >= pEvent.startTS && event.startTS <= timeLimit &&
                        HierarchyHelper.isParentOf(pHierarchy, hierarchy)
                }

                if (parentMatch != null) {
                    childrenMap.getOrPut(parentMatch.first.id!!) { mutableListOf() }.add(event to hierarchy)
                }
            }
            prospectiveParentsByLevel.getOrPut(level) { mutableListOf() }.add(event to hierarchy)
        }
    }
    ...
    // Set to track WHICH children have already been displayed anchored to a SPECIFIC parent instance
    // to avoid redundant injections when B is both a parent and a child.
    val anchoredAddedIds = mutableSetOf<Long>()

    // Helper to extract ListEvent from Event
    fun createListEvent(event: Event, hierarchy: HierarchyInfo?, isGroupedView: Boolean): ListEvent {
        return ListEvent(
            id = event.id ?: 0L,
            ...
            hierarchyTag = hierarchy?.tags?.joinToString("#") ?: "",
            isGroupedView = isGroupedView
        )
    }

    // Recursive helper to add anchored children
    fun addAnchoredChildren(parentEvent: Event, treeProcessedIds: MutableSet<Long>) {
        childrenMap[parentEvent.id]?.sortedBy { it.first.startTS }?.forEach { (childEvent, childHierarchy) ->
            if (!treeProcessedIds.contains(childEvent.id)) {
                treeProcessedIds.add(childEvent.id!!)
                listItems.add(createListEvent(childEvent, childHierarchy, true))
                addAnchoredChildren(childEvent, treeProcessedIds)
            }
        }
    }

    // Helper to find root parent not in current list
    fun startAnchoringFromRoot(event: Event, hierarchy: HierarchyInfo, globalAnchoredSubtrees: MutableSet<Long>) {
        // Find full ancestry path
        val ancestry = mutableListOf<Pair<Event, HierarchyInfo>>()
        var currE = event
        var currH = hierarchy
        
        while (currH.level > 1) {
            val parentLevel = currH.level - 1
            val parents = prospectiveParentsByLevel[parentLevel] ?: break
            val parentMatch = parents.lastOrNull { (pEvent, pHierarchy) ->
                val timeLimit = DateTime(pEvent.startTS * 1000L).plusYears(1).seconds()
                currE.startTS >= pEvent.startTS && currE.startTS <= timeLimit &&
                    HierarchyHelper.isParentOf(pHierarchy, currH)
            } ?: break
            
            ancestry.add(0, parentMatch)
            currE = parentMatch.first
            currH = parentMatch.second
        }

        // Anchoring pull-in should only happen ONCE per root parent per view
        val topAncestor = ancestry.firstOrNull()?.first ?: event
        if (!globalAnchoredSubtrees.contains(topAncestor.id)) {
            globalAnchoredSubtrees.add(topAncestor.id!!)
            
            val treeProcessedIds = mutableSetOf<Long>()
            // If the top ancestor is NOT in today's sorted list, add it as a standalone anchor
            if (sortedEvents.none { it.id == topAncestor.id }) {
                val topHierarchy = HierarchyHelper.extractHierarchy(topAncestor.description)
                listItems.add(createListEvent(topAncestor, topHierarchy, true))
                treeProcessedIds.add(topAncestor.id!!)
            }
            
            // Re-anchor the entire subtree from this root
            addAnchoredChildren(topAncestor, treeProcessedIds)
        }
    }

    // 2. Main collection loop
    val globalAnchoredSubtrees = mutableSetOf<Long>() 
    sortedEvents.forEach { event ->
        val code = Formatter.getDayCodeFromTS(event.startTS)
        
        // Add sections only for the chronological items
        if (addSectionMonths) {
            val monthLabel = Formatter.getLongMonthYear(this, code)
            if (monthLabel != prevMonthLabel) {
                listItems.add(ListSectionMonth(monthLabel))
        ...
        if (code != prevCode && addSectionDays) {
            ...
            listItems.add(ListSectionDay(day, code, isToday, !isToday && event.startTS < now))
            prevCode = code
        }

        // 1. Add the FLAT item (chronological)
        val hierarchy = HierarchyHelper.extractHierarchy(event.description)
        listItems.add(createListEvent(event, hierarchy, false))

        // 2. Anchoring Logic
        if (showSubItems && event.id != null) {
             if (hierarchy != null && hierarchy.level > 1) {
                 // Check if parent is in the current view's chronological list
                 val hasParentToday = sortedEvents.any { pCandidate ->
                     if (pCandidate.id == event.id) return@any false
                     val pHierarchy = HierarchyHelper.extractHierarchy(pCandidate.description)
                     pHierarchy != null && HierarchyHelper.isParentOf(pHierarchy, hierarchy) &&
                        event.startTS >= pCandidate.startTS && event.startTS <= DateTime(pCandidate.startTS * 1000L).plusYears(1).seconds()
                 }
                 
                 if (!hasParentToday) {
                     // ORPHAN: Pull in ancestors and build full tree once
                     startAnchoringFromRoot(event, hierarchy, globalAnchoredSubtrees)
                 } else {
                     // PARENT IS PRESENT: Also anchor children to THIS flat instance for redundancy/clarity
                     addAnchoredChildren(event, mutableSetOf())
                 }
             } else {
                 // ROOT ITEM: Anchor its children
                 addAnchoredChildren(event, mutableSetOf())
             }
        }
    }
...
fun Context.updateTaskCompletion(event: Event, completed: Boolean, recursive: Boolean = true) {
    val eventId = event.id ?: return
    if (completed) {
        event.flags = event.flags or FLAG_TASK_COMPLETED
        val task = Task(null, eventId, event.startTS, event.flags)
        completedTasksDB.insertOrUpdate(task)
    } else {
        event.flags = event.flags and FLAG_TASK_COMPLETED.inv()
        completedTasksDB.deleteTaskWithIdAndTs(eventId, event.startTS)
    }
    ...
    // update main events db status for non-repeating tasks, or for CalDAV
    eventsDB.updateTaskCompletion(eventId, event.flags)
    updateWidgets()

    if (recursive) {
        updateChildStatuses(event, completed)
    }
}

fun Context.toggleCompletion(event: Event, completed: Boolean, recursive: Boolean = true, startTS: Long = 0L) {
    val eventId = event.id ?: return
    if (event.isTask()) {
        updateTaskCompletion(event, completed, recursive)
    } else {
        if (completed) {
            ...
            if (event.title.startsWith(COMPLETION_PREFIX)) {
                event.title = event.title.substring(COMPLETION_PREFIX.length)
            }
        }

        if (event.repeatInterval != 0 && startTS != 0L) {
            val duration = event.endTS - event.startTS
            event.startTS = startTS
            event.endTS = startTS + duration
            eventsHelper.editSelectedOccurrence(event, startTS, false) {}
        } else {
...
private fun Context.updateChildStatuses(parent: Event, completed: Boolean) {
    ensureBackgroundThread {
        val allEvents = eventsDB.getAllEvents()
        recursiveUpdate(parent, completed, allEvents)
    }
}

private fun Context.recursiveUpdate(parent: Event, completed: Boolean, allEvents: List<Event>) {
    val hierarchy = HierarchyHelper.extractHierarchy(parent.description) ?: return
    val parentLevel = hierarchy.level

    val children = allEvents.filter {
        val childHierarchy = HierarchyHelper.extractHierarchy(it.description)
        if (childHierarchy == null) return@filter false

        // 1-year constraint
        val timeLimit = DateTime(parent.startTS * 1000L).plusYears(1).seconds()
        it.startTS >= parent.startTS && it.startTS <= timeLimit &&
            HierarchyHelper.isParentOf(hierarchy, childHierarchy)
    }

    children.forEach { child ->
        toggleCompletion(child, completed, recursive = false, startTS = child.startTS)
        recursiveUpdate(child, completed, allEvents)
    }
}
